...
import { ALERTA_BADGE, alertaTipoTexto, alertaEstadoTexto, alertaTexto, CATEGORIAS_ALERTA, alertaCategoriaInfo } from '../../utils/constantes.js';
...
export async function init() {
  ...
  function renderAlertas() {
    const filtradas = alertas
      .filter(a => filtroAlerta === 'todas' || a.tipo === filtroAlerta)
      .filter(a => filtroCategoria === 'todas' || CATEGORIAS_ALERTA[a.categoria] && a.categoria === filtroCategoria);

    const contar = (tipo) => alertas.filter(a => a.tipo === tipo).length;
    const contarCategoria = (categoria) => alertas.filter(a => a.categoria === categoria).length;
    ...
    container.innerHTML = `
      ...
      <div class="solicitudes-filtros" style="margin-bottom: var(--space-3);">
      ...
      <div class="solicitudes-filtros" style="margin-bottom: var(--space-6);">
        <button class="filtro-btn ${filtroCategoria === 'todas' ? 'active' : ''}" data-categoria="todas"><i class="fa-solid fa-layer-group" style="margin-right: 4px;"></i>${t('all')} (${alertas.length})</button>
        ${Object.keys(CATEGORIAS_ALERTA).map(categoria => `
          <button class="filtro-btn ${filtroCategoria === categoria ? 'active' : ''}" data-categoria="${categoria}">
            <i class="fa-solid ${CATEGORIAS_ALERTA[categoria].icono}" style="margin-right: 4px;"></i>${t(`cat_${categoria}`)} (${contarCategoria(categoria)})
          </button>
        `).join('')}
      </div>
      ...
      <div class="alertas-list">
        ${filtradas.length > 0 ? filtradas.map(alerta => {
          ...
          const categoria = alertaCategoriaInfo(alerta);
            ...
            <div class="alerta-card alerta-card--${alerta.tipo}">
              ...
              <div class="alerta-card__body">
                ...
                <div class="alerta-card__footer">
                  ${categoria ? `<span class="badge badge-neutral"><i class="fa-solid ${categoria.icono}" style="margin-right: 4px;"></i>${categoria.etiqueta}</span>` : ''}
    ...
    // Bind filtros
    container.querySelectorAll('.filtro-btn[data-filtro]').forEach(btn => {
    ...
    // Bind filtros por categoría
    container.querySelectorAll('.filtro-btn[data-categoria]').forEach(btn => {
      btn.addEventListener('click', () => {
